...
def test_settings_rejects_timeout_shorter_than_latency_threshold() -> None:
    with pytest.raises(RuntimeError, match="PROBE_TIMEOUT_SECONDS"):
        settings(probe_timeout_seconds=1, latency_threshold_ms=1000)
...
def test_probe_success_is_good_for_availability_and_latency() -> None:
    ...
    assert probe_result_to_sli_samples(result, settings()) == SliSamples(1, 1, 1, 1)


def test_probe_slow_success_is_availability_good_latency_bad() -> None:
    cfg = settings(latency_threshold_ms=1000)
    result = probe_endpoint(
        cfg,
        urlopen=lambda *_args, **_kwargs: FakeResponse(200),
    ...
    assert probe_result_to_sli_samples(result, cfg) == SliSamples(1, 1, 0, 1)


def test_probe_http_error_is_bad_sample() -> None:
    ...
    assert probe_result_to_sli_samples(result, settings()) == SliSamples(0, 1, 0, 1)


def test_probe_url_error_is_bad_sample() -> None:
    result = probe_endpoint(
        settings(),
...
def test_metric_samples_use_publish_timestamp_and_sli_labels() -> None:
    cfg = settings()
