...
- 注意: `http.server.active_requests` は OTel UpDownCounter (DELTA) で per-process state を持つため、Chaos 実験で Pod 再起動が頻発する環境では `rate()` / `delta()` 型のクエリで負値や jitter が出ます。負荷状態の一次信号は Envoy 経由の `gateway:chaos_app:http_request_rate` recording rule を使ってください (詳細は [docs/workarounds.md §D-5](docs/workarounds.md#d-5-opentelemetry-updowncounter-httpserveractive_requests-の-pod-再起動時ドリフト))。
