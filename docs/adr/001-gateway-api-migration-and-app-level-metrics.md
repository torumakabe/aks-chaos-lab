...
- **Bicep API `2026-03-01` を使用**: `webAppRouting.gatewayAPIImplementations.appRoutingIstio` プロパティが必要。`2025-08-02-preview` では Istio 有効化プロパティが欠落し IaC 管理外の CLI 操作が必要になるため却下。
...
- **制約**: Envoy のメトリクスが AKS コントローラーのリコンサイルで維持できないため、アプリ層メトリクスへの移行を強いられる。その結果、HTTPChaos 実験は Envoy が障害をアプリ到達前に注入するため、アプリ側のメトリクスには記録されない。障害の観測には Locust など外部クライアントのメトリクスを使う。
