...
- **制約 (no-traffic 時の SLI 信号空白)**: Managed Prometheus の `gateway:chaos_app:*` recording rule は raw 入力 series が無いと NaN を返し、Azure Monitor SLI WindowBased 評価で no-data が Bad 扱いされ得る。`infra/modules/prometheus/recording-rules.bicep` の R1-R5 を `clamp_min(..., 1e-9)` (R2/R3 ratio) / `or on(cluster_name) (... * 0)` (R1 p95) / `increase(...[5m])` (R4/R5 totals) に変更し、no-traffic 時にも安定した値を出すようにした。実機検証 (eval, 2026-05-07) で `synthetic-traffic` CronJob を 8 分間 suspend した状態でも R2/R3 ratio = 0/1、R4/R5 total = 0、`http_request_rate` = 0 が安定して返ることを確認した。R1 p95 については `histogram_quantile` が `envoy_cluster_external_upstream_rq_time_bucket` の cumulative bucket を参照する性質上、5 分窓の rate=0 でも NaN にはならず直近の良好値 (例 9.75ms) を維持する。SLI 評価上は "low latency = good" 方向で安全側に倒れるため許容するが、`or on(cluster_name) (... * 0)` は histogram 自体が消失したクリーン環境直後の保護として残す。
- **制約 (合成トラフィック CronJob)**: SLI 信号源を常時供給するため `k8s/base/cronjob-synthetic-traffic.yaml` で 1 req/min の合成トラフィックを `chaos-app-approuting-istio.chaos-lab.svc.cluster.local` (AKS Istio addon の Gateway controller が作る per-Gateway LoadBalancer Service) 経由で `/` に流す。Cilium L7 Egress NetworkPolicy は機能軸で chaos-app egress と同じ `k8s/base/ciliumnetworkpolicy-egress-allowlist.yaml` に統合し、Pod 単位で `app: synthetic-traffic` を allowlist 化する。負荷テスト時など一時無効化したい場合は `kubectl -n chaos-lab patch cronjob synthetic-traffic --type=merge -p '{"spec":{"suspend":true}}'` で suspend 可能 (suspend 中は `ChaosAppNoTraffic` alert が発火しうる)。
