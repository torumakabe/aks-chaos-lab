...
- **制約 (合成トラフィック CronJob)**: SLI 信号源を常時供給するため `k8s/apps/chaos-app/cronjob-synthetic-traffic.yaml` で 1 req/min の合成トラフィックを `chaos-app-approuting-istio.chaos-lab.svc.cluster.local` (AKS Istio addon の Gateway controller が作る per-Gateway LoadBalancer Service) 経由で `/` に流す。Cilium L7 Egress NetworkPolicy は機能軸で chaos-app egress と同じ `k8s/apps/chaos-app/ciliumnetworkpolicy-egress-allowlist.yaml` に統合し、Pod 単位で `app: synthetic-traffic` を allowlist 化する。負荷テスト時など一時無効化したい場合は `kubectl -n chaos-lab patch cronjob synthetic-traffic --type=merge -p '{"spec":{"suspend":true}}'` で suspend 可能 (suspend 中は `ChaosAppNoTraffic` alert が発火しうる)。
