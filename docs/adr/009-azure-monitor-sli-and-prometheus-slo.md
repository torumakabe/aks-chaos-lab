...
- Latency SLI の主信号は、保存済み p95 ではなく、5分窓内で 1秒以内に完了したリクエスト割合 `gateway:chaos_app:http_request_duration:le_1s_ratio` とする。`gateway:chaos_app:http_request_duration:p95` は operational / debug 用に維持する。
