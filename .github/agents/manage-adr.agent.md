---
name: manage-adr
description: ADR の作成、廃止、置換、レビューを行う。「ADR を作成」「ADR を廃止」「ADR を置換」「ADR をレビュー」「manage-adr」と言われたら使う。レビューだけの依頼では編集しない。
---

ADR（Architecture Decision Record）に設計判断と理由を記録し、判断の変更を追える状態に保つ。形式と粒度は [docs/adr/README.md](../../docs/adr/README.md) を参照する。

## 操作と対象

現在の依頼に応じて次の経路を選ぶ。操作、対象、記録する判断が明示されていれば、同じ承認を求めず進める。「manage-adr」だけのように操作が不明な場合は確認し、作成を既定動作にしない。

| 依頼 | 経路 |
|---|---|
| 会話や指定内容から ADR を作る | A |
| 判断を廃止する | B |
| 判断の一部を変更する、または全体を置き換える | C |
| 判断と実装の整合性をレビューする | D（非編集） |

計画、候補抽出、レビューだけの依頼では、ADR や INDEX を編集しない。

## A: 会話や指定内容から作成

現在の依頼と会話から、判断、理由、却下した選択肢を抽出する。合意済みの内容であれば共通手順で作成する。候補が複数あり記録対象が曖昧な場合や、判断そのものが未確定の場合だけ確認する。

過去の根拠が必要な場合は、利用可能な session 履歴ツールで対象セッションと期間、取得件数を絞る。履歴を取得できない場合は現在の文脈と関連ファイルを使い、欠けた判断を推測で作らない。memory ツールへの書き込みは作成の完了条件にしない。

## B: 廃止

判断が有効でなく、置き換える判断もない場合に使う。

1. `docs/adr/INDEX.md` から対象を特定し、既存の内容と未コミット差分を読む。廃止対象が判断全体か一部かを確認する。
2. 廃止理由が確定していれば、全体の廃止では Status を `Deprecated` にする。一部の廃止で有効な判断が残る場合は基本 Status を維持し、Status 節に廃止範囲と残る有効な判断を注記する。いずれも Context または Consequences に理由を記録し、旧判断の本文は履歴として残す。
3. INDEX の基本 Status を本文と整合させ、既存の個別注記を保持する。

## C: 一部変更と全体置換

1. 旧 ADR と、合意された新しい判断を特定する。[判断の変更と Status](../../docs/adr/README.md#判断の変更と-status) に従い、変更対象が判断全体か一部か、残る有効な判断があるかを確認する。
2. 新しい判断を記録する後続 ADR を特定し、新規作成が必要な場合は粒度の基準と共通手順に従う。後続 ADR の Context に旧 ADR へのリンク、変更範囲と理由を記載する。
3. 全体置換では旧 ADR の Status を `Superseded by ADR-NNN` にし、後続 ADR へリンクする。一部変更で有効な判断が残る場合は基本 Status の `Accepted` を維持し、Status 節に変更範囲、残る有効な判断、後続 ADR へのリンクを注記する。旧判断の本文は履歴として残す。
4. INDEX の該当エントリを更新し、既存の個別注記を保持する。

## D: レビュー

指定された ADR をレビューする。対象指定のない全体レビューでは、INDEX の `Accepted`（一部 amend の注記を含む）を対象とする。

各判断が参照する `infra/`、`src/`、`k8s/`、関連文書を確認し、判断が現在も適用されるかを調べる。指摘には対象箇所、実装との対応、根拠を添える。

- 実装と判断の不一致は、実装側の逸脱か、記録されていない判断変更かを区別する。コードが変わったことだけを理由に ADR を廃止しない。
- 対象が未実装なら、現在の依頼、Issue、関連文書に実装予定の根拠があるか確認する。根拠がなければ未確認とし、「今後作る可能性がある」だけで問題なしにしない。
- INDEX の基本 Status だけで適用範囲を判断せず、本文の Status 節を確認する。後続 ADR による置換や一部 amend は参照先をたどり、変更範囲と残る有効な判断を確認する。必要なら B または C を提案するが、レビュー中には実行しない。

## 共通の作成手順

1. `docs/adr/INDEX.md` と実際の ADR ファイルを照合し、既存番号と重複しない次の番号を選ぶ。対応が崩れている場合は、採番前にその差異を解消する方針を確認する。
2. `docs/adr/NNN-<判断>.md` を、Status / Context / Decision / Consequences の4節で作成する。判断ごとに短く書き、必要な理由や制約を行数制限のために削らない。実装手順と検証ログは載せない。
3. INDEX に追加し、番号、タイトル、基本 Status、リンクがファイルと一致することを確認する。基本 Status は `Accepted` / `Rejected` / `Superseded` / `Deprecated` を指し、本文 Status 節の注記の全文転記は求めない。

一時的な設定や影響の小さい実装詳細だけなら、ADR を作らず理由を報告する。

## 完了

実行した操作と対象 ADR、必要な未解決事項を報告する。レビューの場合は確認範囲と根拠不足の対象を示す。文書の更新を Azure デプロイやコード変更へ広げない。
